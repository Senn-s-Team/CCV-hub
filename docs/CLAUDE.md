...
system/: 系统设计目录，存放组件划分、运行流程、数据流、异常处理与阶段性收口基线
api/: 接口契约目录，存放本地服务接口、请求响应结构与实例模型
design/: UI/UX 设计目录，存放总规范、页面原型规则、状态表达与评审基线
adr/: 架构决策目录，存放关键技术决策、阶段边界与取舍理由
deploy/: 开源部署目录，存放部署总览、模式拆分、平台适配文档、release 方案、验证清单、故障排查与开发进度记录
...
11. `docs/adr/0004-stable-viewer-path-entry.md`
12. `docs/deploy/overview.md`
13. `docs/deploy/modes.md`
14. `docs/deploy/release-plan.md`
15. `docs/deploy/release-checklist.md`
16. `docs/deploy/troubleshooting.md`
17. `docs/deploy/progress.md`
18. `docs/system/tech-stack.md`
...
- `deploy/`：定义开源部署架构、平台适配、release 产物、进度拆分与运维验证。
...
- 不引入 SSE 作为实例列表同步主路径。
- 不引入数据库作为首版主存储。
- Dokploy / Traefik / Hub bridge 已由 ADR 0003 纳入当前 viewer bridge 实现，ADR 0004 规定新入口使用稳定 Hub host path。

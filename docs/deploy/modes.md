...
- viewer path 通过 Compose Web 入口或上游边缘代理转发到 Agent bridge。
...
- Traefik 管理 Hub 主域名，viewer 复用同 host `/viewer/*`。
...
- viewer path 经 Traefik 和 nginx 进入 Agent bridge。
...
```text
Caddy
  |-- <CCV_HUB_PUBLIC_HOST> -> static web and /api proxy
  |-- <CCV_HUB_PUBLIC_HOST>/viewer/* -> agent bridge
```

职责：

- Caddy 提供 HTTPS 与同 host viewer path 路由。
...
- viewer `/viewer/` location 与 `CCV_HUB_VIEWER_PATH_PREFIX` 一致。
...
- Ingress 支持 Hub 主域名，viewer 复用同 host path。

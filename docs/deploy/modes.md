...
- viewer 子域名通过 Compose 网络或边缘代理转发到 Agent bridge。

## 3. Dokploy Mode

Dokploy Mode 是 Docker Compose Mode 的平台适配。
...
- 需要使用 Dokploy 网络名或 labels。
- 需要处理 `host.docker.internal:host-gateway`。
- 需要给 `ccv-*` viewer HostRegexp 设置低优先级路由。
...
- WebSocket upgrade 和 SSE 不被 buffering 破坏。

## 4. Caddy Mode

Caddy Mode 面向希望自动 HTTPS 和最少配置的自托管用户。
...
Nginx Mode 面向传统 VPS、已有 Nginx 和 Certbot 的用户。
...
Kubernetes Mode 是团队和高级用户场景。

推荐边界：

```text
Kubernetes: ccv-hub-web, ingress, config
Host node: ccv-hub-agent
```

Agent 可以未来探索 DaemonSet，但前提是每个节点都有 Claude Code 环境、项目目录、`cc-viewer` CLI 和明确的路径授权。
...
该模式不作为默认开源路径。它能减少 systemd 依赖，但会显著扩大容器权限面。

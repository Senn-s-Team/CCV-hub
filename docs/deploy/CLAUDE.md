...
成员清单
overview.md: 开源部署总览，定义 ccv-hub-web 控制面、ccv-hub-agent 执行面、同 host `/viewer/*` bridge、平台适配器、默认部署拓扑与 env source matrix
...
troubleshooting.md: 部署故障排查文档，定义 Agent、鉴权、API、公网 404、Agent proxy token drift、viewer path bridge、SSE、WebSocket、停止与回滚问题的检查路径
progress.md: 开源部署改进进度，记录阶段任务、状态、验证证据、rehearsal 入口、阻塞项与下一步执行顺序
compose.md: Docker Compose 开源主路径文档，定义 image-only Web 容器、宿主机 systemd Agent、hub/standalone compose 适用边界、viewer path 变量配置与 smoke path

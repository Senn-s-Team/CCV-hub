...
Dokploy 管理 Web 容器和 Hub 主域名路由，viewer 通过同 host `/viewer/*` 进入 Web nginx。Agent 继续由宿主机 systemd 管理。
...
Agent 的 `/etc/ccv-hub/.env.agent` 使用同一组公网域名、viewer path 前缀和 `CCV_HUB_AGENT_PROXY_TOKEN`。Dokploy 容器通过 `host.docker.internal:4318` 回连宿主机 Agent，Agent 需要监听容器可达地址，推荐 `CCV_HUB_HOST=0.0.0.0` 并用防火墙阻止公网直连 `4318`。
...
3. 新启动实例生成 `https://<CCV_HUB_PUBLIC_HOST>/viewer/<id>/?token=...` viewer URL。
4. viewer path 经 Traefik 进入 Web nginx，再进入 Agent bridge。

...
- 地址形态固定为 viewer 子域名，例如 `https://ccv-<bridgeId>.<CCV_HUB_PUBLIC_DOMAIN>/?token=<token>`
...
用户在 Hub 页面选择项目 → 宿主机 `ccv-hub-service` 以 `opc` 用户环境启动 `ccv` → `cc-viewer` 绑定端口并生成访问 token → Hub 生成 `https://ccv-<bridgeId>.<CCV_HUB_PUBLIC_DOMAIN>/?token=<token>` → `ccv-hub` 登记并展示该地址

/**
 * [INPUT]: 依赖 @ccv-hub/shared-contracts 的实例类型与内部状态定义，依赖停止句柄推导公开可停止能力
 * [OUTPUT]: 对外提供 ManagedInstanceRecord、CreateManagedInstanceInput 与 toPublicInstance，内部保留 viewer upstream 地址与 stop handle
 * [POS]: hub-service 的领域模型中心，负责区分内部记录与页面可见实例，统一生成 canStop 投影
 * [PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
 */
import type { Instance, InternalInstanceStatus } from '@ccv-hub/shared-contracts';

export type CreateManagedInstanceInput = Omit<Instance, 'status' | 'canStop'> & {

...
成员清单
auth-session.ts: 面板会话模块，负责环境变量解析、管理员口令校验、HMAC session token 签发、验证与 cookie 属性配置
bridge-url.ts: viewer bridge 地址模块，负责稳定 Hub host path 生成、path 解析、path prefix 剥离、保留 Hub path 避让与 upstream token 注入规则

...
成员清单
auth.ts: 面板鉴权路由，挂载登录、登出、登录态接口，并保护公网面板 API 与 viewer bridge 页面
...
instances.register.ts: 外部实例注册路由，挂载 `POST /api/instances/register`，接收 cc-viewer 插件上报的手动启动实例并避免覆盖 hub 启动实例
instances.unregister.ts: 外部实例注销路由，挂载 `POST /api/instances/unregister`，接收 cc-viewer 插件上报的手动停止事件并只移除 manual 实例
viewer-bridge.ts: viewer 子域名桥接路由，按 `ccv-*` Host 反代已鉴权 HTTP/SSE/WebSocket 到对应 cc-viewer upstream

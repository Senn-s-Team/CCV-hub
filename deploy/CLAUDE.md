...
成员清单
docker-compose.hub.yml: ccv-hub-web 的 image-only release 公网入口清单，定义 Web image tag、host.docker.internal 回连宿主机 Agent、可配置 Docker 网络、Hub 域名路由与 viewer 子域名路由；release 通过环境变量替换 image、域名、端口和 viewer 前缀
